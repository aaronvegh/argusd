...
env CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-X main.Version=$NEW_VERSION" -a -o argusd-linux-arm64
...
env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 go build -ldflags="-X main.Version=$NEW_VERSION" -a -o argusd-linux-arm
...
aws s3 cp argusd-linux-arm64 s3://argusd/argusd/ --acl public-read
...
aws s3 cp argusd-linux-arm s3://argusd/argusd/ --acl public-read
